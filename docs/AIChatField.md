...
- **高さ**: `IsFillAvailable: true` のグリッドの最終行に置くと残りの高さを使い切り、履歴だけが内部スクロールする。普通の行では履歴の領域は 16rem 固定で、返事が増えても行は伸びない (行が伸びると下の行やページを押し出す)。`Height` (px) の指定も可
...
| Height | int | - | 高さ (px)。0 なら親の高さに合わせる (FillAvailable の最終行では残り全部、普通の行では履歴 16rem ぶん)。それ以外の高さにしたいときだけ指定する |
