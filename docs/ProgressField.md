...
- **色の参照**: `ColorField` に指定したフィールド (Text / ColorPicker) の色をバー/メーター色に使用。未設定/空なら `BarColor` (固定色) を使用
- **バー: ガント風の見た目**: 全体は薄い同色、完了部分は濃い同色ベタ塗り (Gantt のタスクバーと同じ)
- **メーター: 半円ゲージ**: 薄い同色の円弧の上に、値のぶんだけ濃い同色の円弧を描画。中央に進捗率を表示 (色はバーと同じ色)
- **進捗率の表示**: バーは重畳表示、メーターは中央表示。`ShowValueLabel` で表示 ON/OFF
- **見やすい文字色 (バー)**: ラベルの文字色は背景色に対する YIQ コントラスト色を自動選択 (Gantt と同じロジック。明るい背景→濃色、暗い背景→白)
...
`ValueField` の値は `Scale` に従って 0〜100% に換算し、範囲外はクランプして表示します。`Scale = Percent` なら値をそのまま%、`Ratio` なら 1.0 を 100% とみなします (GanttField の `ProgressScale` と共通)。
...
> メーター中央の数値は SVG の `<text>` を動的に描画する都合上、色・サイズは要素の `style` 属性で指定しており、scoped CSS では上書きできません。色はバー色 (100% 時の色) に追従します。
