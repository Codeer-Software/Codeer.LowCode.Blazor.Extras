...
> **非推奨。** 本体 (Codeer.LowCode.Blazor) の**フォーカス制御機能** (`DetailLayoutDesign.FocusControlMode` など) が後継です。
> Enter / Tab によるフォーカス遷移に加え、遷移順序・末尾挙動・初期フォーカス・遷移スキップなどを本体側で細かく制御できます。
> 既存モジュールに配置済みの本フィールドは動作し続けますが、デザイナのツールボックス・フィールド種別選択では非表示になります。新規では本体のフォーカス制御機能を使用してください。
