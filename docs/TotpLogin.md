...
`Codeer.LowCode.Blazor.Extras.Server` の `Auth` 名前空間 + フィールド `TotpSecretField`。ID/パスワードのログイン (Starter の Cookie テンプレート) に、
オーセンティケータアプリ (Google Authenticator / Microsoft Authenticator 等) の 6 桁コードによる二要素認証を足す。
RFC 6238 (HMAC-SHA1 / 30 秒 / 6 桁)。QR コードは QRCoder。
...
`POST api/account/login` が 2 段階になる。1 段階目は `{ Id, Password }`、2 段階目は同じものに `TotpCode` を足して送る。
応答は `{ status, secret?, otpauthUri?, qrPngBase64? }`。

| status | 意味 | クライアントの動き |
|---|---|---|
| `setup` | パスワードは正しいが未登録 | QR (`qrPngBase64`) と手入力用の鍵 (`secret`) を表示し、コードを入力させて `TotpCode` 付きで再送 |
| `totp` | パスワードは正しく登録済み | コードを入力させて `TotpCode` 付きで再送 |
| `invalid_code` | コード不一致 (期限切れ・同じコードの再利用を含む) | 入力し直し |
| `ok` | サインイン済み | アプリへ |
