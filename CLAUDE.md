...
- **Agent の選択**: デザインの `Agent` (文字列、空 = 既定) を `AIChatSendRequest.Agent` で送り、`AIChatService` がコンストラクタで受けた対応表 `Func<string, IAIChatAgent?>` で解決 (アプリ側の静的クラス。Example は `AI/AIChatAgentTable.cs` = MailSenderTable と同型、名前ごとに Lazy で 1 つ、`Service` 静的プロパティに AIChatService)。無い名前は error。DI 登録なし。`AIChatService.Start(owner, conv, message, agentName)` (internal)
...
- **サーバー** (Extras.Server `AI/Chat/`。フォルダ = 役割: 直下 = 共有の契約と窓口 (`IAIChatAgent` / `AIChatService` / `ChatReplyHtml`)、`ChatClient/` = 内部の会話エンジン (ChatClientAgent とツールセット機構、Chart ツール。すべて internal)、`RawDataAccess/` = 専用 (Agent / Options / ToolSet / DbSchemaReader)、`DesignKnowledge/` = 設計参照ツール (DesignDescriber = DesignData の要約、DesignKnowledgeToolSet = list_modules / describe_module / read_document)。名前空間もフォルダどおり): `IAIChatAgent` / `AIChatService` (旧 AIChatJobStore。プロセス内ジョブ。プロセスに 1 つ。対応表 Func か単一 Agent を受ける。FinishedRetention / MaxRunning は init プロパティ = Options 型は無い) / `ChatReplyHtml` (Markdig で Markdown→HTML、テキストはエスケープ、HTML 素通し + target=_blank) / `ChatClientAgent` (Microsoft.Extensions.AI `IChatClient`。システムプロンプト + internal な `IAIChatToolSet` 群、会話履歴 `ConversationHistory` (鍵 = 所有者+会話ID (所有者空なら会話IDのみ)。切り詰め 3 段: 古いターンのツール呼出/結果を落とす KeepToolResultsForTurns=2 → ターン数 MaxHistoryTurns=20 → 文字数 MaxHistoryCharacters=40000。いずれもターン境界で)、FunctionInvokingChatClient で往復上限、ストリーミング → ReportPartial、Markdown→HTML→ToolSet.PostProcessHtml) / `RawDataAccess/RawDataAccessToolSet` (get_schema = `DbSchemaReader` で DB 種別ごとのカタログ問い合わせ + モジュール定義の業務名、execute_sql = 1 文 SELECT 判定 `Validate` + DbCommand 直叩き (タイムアウト・MaxRows・MaxResultChars) + ログ) / `ChatClient/ChartToolSet` + `SvgChart` (render_chart → `[[chart:N]]` プレースホルダ → 後処理で `<div class="aichat-chart"><svg>`) / `RawDataAccessAgent` (ChatClientAgent を包含して委譲 + 上 2 ツールセット。継承ではない。ChatClientAgent は sealed)
- **公開面**: public は契約 (`IAIChatAgent` / `AIChatAgentRequest` / `AIChatReply` / `AIChatReplyFormat` / `IAIChatProgress`)、窓口 (`AIChatService`。★2026-09-24 AIChatJobStore + Options を 1 型に畳んだ破壊的変更)、標準 Agent (`RawDataAccessAgent` + `RawDataAccessOptions`。意味検索は `semanticSearch: SemanticSearchService?` で受ける。会話の設定 (プロンプト・履歴上限・逐次表示) も RawDataAccessOptions に、ILoggerFactory はコンストラクタ) だけ。`ChatClientAgent` / `ChatClientAgentOptions` / `IAIChatToolSet` / `AIChatToolContext` / `RawDataAccessToolSet` / `ChartToolSet` / `SvgChart` / `ChatReplyHtml` / `ConversationHistory` は internal (テストは IVT)。独自ツールの Agent はアプリが `IAIChatAgent` を直接実装する (ToolSet を公開しない = 実装の詳細)
...
- **ローカライズ**: 画面に出る文言 (進捗・未登録エラー・グラフの「データなし」) は Extras.Server の `Properties/Resources(.ja-JP).resx`。AIChatService / SemanticSearchService が Start 時のカルチャをバックグラウンドへ引き継ぐ。モデル向けのプロンプトとツール説明は日本語の定数 (ローカライズ対象外)
- **テスト**: `Test/AI/` (AIChatServiceTest = ジョブ + 対応表の振り分け / ChatClientAgent は台本つき `FakeChatClient` / RawDataAccessToolSet は SQLite 実 DB / SvgChart)
...
- **状態**: 実装済み (docs/SemanticSearchField.md, Extras.Designer FieldDocs/SemanticSearchFieldDesign.md)。Example にはホスト側の結線だけ (`AI/SemanticSearchIndex.cs` = SemanticSearchService の静的な持ち物 (`Service`。プロバイダは Lazy で EmbeddingProviderTable から) + `AI/EmbeddingProviderTable.cs` / `Controllers/SemanticSearchController.cs` = 再索引 API / CustomizedModuleDataIO / AIChatAgentTable)。Example のデータは SQLite なので意味検索できるモジュールは置いていない (`Inquiry` は通常モジュール)
...
- **テスト**: `Test/AI/` (FakeEmbeddingProvider = 2-gram ハッシュ 128 次元 / SemanticSearchTextTest / SemanticSearchIndexingDbTest (SQLite・索引付けとデザインチェック) / SemanticSearchToolSetTest (接続せず判定と説明) / SemanticSearchDbSearchTest (SQL 文字列) / SemanticSearchVectorDbTest は [Explicit]・実 DB (FakeEmbedding で 保存→再索引→search_records→{embed:} SQL 実行。環境変数 SEMANTIC_SEARCH_PG_CONNECTION = pgvector 済み PG / SEMANTIC_SEARCH_MSSQL_CONNECTION = SQL Server 2025。2026-09-23 に Docker の pgvector/pgvector:pg16 と mcr.microsoft.com/mssql/server:2025-latest で両方緑 = 生成列 ::vector / VECTOR_DISTANCE / cast(is_deleted as integer) の構文確認済) / SemanticSearchRealAITest は [Explicit]・実 Azure OpenAI + pgvector PG (環境変数 AZURE_OPENAI_* と SEMANTIC_SEARCH_PG_CONNECTION))
...
#### EditHistoryField / EditHistoryContractField - 編集履歴 (2026-09-25)
- **状態**: 実装済み (docs/EditHistory.md, Extras.Designer FieldDocs/EditHistoryFieldDesign.md + EditHistoryContractFieldDesign.md)。Example とテンプレート (Starter 4 ホスト) の CustomizedModuleDataIO に結線済み
- **構図**: 承認フローと同じ契約フィールド方式。対象モジュール側 = `EditHistoryField` (UI + 「履歴を取る」マーカー + `HistoryModuleName` / `LayoutName` / `PageSize`)、履歴モジュール側 = `EditHistoryContractField` (役割 ModuleName / DataId / ChangeType / Snapshot 必須、UserId / DateTime 任意)。ModuleName は Text / Select (Select + デザイン enum で一覧の表示・検索をモジュール名から画面上の名前に読み替え。enum は任意・空なら素の名前。enum にメンバーがあるのに記録元が無いとチェックエラー = 承認待ち一覧の申請種別と同じ作り、Example は `EditHistoryTargetModule`)。`EditHistoryChangeType` は `[DesignEnum]` (Add / Update / Delete)
- **記録 (Extras.Server `EditHistory/EditHistoryRecorder`)**: ホストの `SubmitAsync` override が `_editHistory.SubmitAsync(transactionData, () => base.SubmitAsync(...))` で包む。ルート 1 件 = 履歴 1 行。作成・更新は base 後に本体 `ModuleDataIO.GetWithOwnedRecordsAsync` で DB から読み直し (従属レコードの宣言を再帰、親の実値で束縛、ページングなし)、削除は base 前に読む。書き込みは `AddSystemRecordAsync` (内部経路)。失敗は結果の ExceptionMessage = ロールバック。一括 INSERT 経路 (Id が返らない) はスキップしてログ。ExecuteSqlField: Create/Update/Delete タイミングは通常どおり (Create は NewId 必須)、Standalone 送信 (`IsStandalone`) は前後のスナップショットを比べて変わったときだけ記録。★版番号は保存しない (記録側が履歴モジュールを読むと、履歴を読めないユーザーの保存で採番が狂う) = 閲覧時に TotalCount から採番
- **閲覧 (`Fields/EditHistoryField`)**: Detail の InitializeDataAsync で `IModuleDataService.GetListAsync` (ModuleName == 自モジュール && DataId == 自 Id、DateTime→Id 降順、LimitCount = PageSize)。同じ呼び出しで「次の 1 行」(LimitCount=1, PageIndex=行オフセット) も読み、ページ末尾の版の差分の元にする。差分 = `EditHistory/EditHistoryDiff` (前版スナップショットと比較。明細は行 Id で突き合わせ。閲覧権限のない項目 = `FieldBase.HasUserReadPermission` false は出さない)。値の整形 = `EditHistoryValues` (Select/Link は DisplayText、IExternalTextFormatFieldDesign の書式)。★旧→新を出すのは `IsTextSupported` の型 (文字列/数値/真偽/日付時刻/File) だけ。独自データクラスは JSON 比較で有無だけ (`HasValueText=false`・「変更あり」)、内容は「この版を表示」に任せる (ユーザー決定 2026-09-26「1で」)
- **この版を表示**: `CreateChildModuleAsync` (onInitialize で ListField / Gantt 等の AllowLoad=false) → `SetDataWithoutInteractionAsync(snapshot)` → 従属レコードを宣言した拡張フィールドに `IOwnedRecordsField.ShowOwnedRecordsAsync` で版の行を渡す (`OwnedRecordsDisplay` が表示専用の子モジュールを作る・強調無し) → `IsViewOnly=true` → 変更フィールドに `FieldBase.ClassName` で `edit-history-changed` を付け `ShowDialogAsync`。強調 CSS はダイアログが本コンポーネントの外に描画されるためコンポーネント内の `<style>` でグローバルに出す (scoped CSS では届かない)
- **この版に戻す (`EditHistory/EditHistoryRestorer`)**: 値は `SetDataAsync` (変更扱い・スクリプト発火)、明細は Rows を Id で突き合わせ `InsertRowsAsync` (システム値を外して新規行) / `DeleteRowAsync` (ConfirmBeforeDelete=true の一覧は行ごとに確認が出る = 既知)。対象外 = システムフィールド・リンク越し・参照系一覧・FileField・書き込み権限なし。保存はユーザー。★`ListField.SetDataAsync` は全行を消して新 Id で入れ直す (Id マッチにならない) ので使わない
- **スナップショット (`EditHistory/EditHistorySnapshot`)**: `JsonConverterEx.SerializeObject(data, false)` (TypeFullName 式)。FileFieldData.Content は落とす。Extras (client) に置き Extras.Server から IVT で共用
- **テスト**: `Test/DesignCheck/EditHistoryDesignCheckTest` (契約/型/重複/リネーム)、`Test/EditHistory/EditHistoryDiffTest`、`EditHistoryRecorderDbTest` (SQLite: 作成/更新/明細のみ更新/削除/対象外/設定不備で失敗/任意役割)、`EditHistoryFieldHarnessTest` (条件・採番・ページ末尾の差分・復元の Id マッチと新規行)
- **従属レコードの宣言 (2026-09-26 ユーザー方針「本体は部品、ポリシーは外」)**: 本体 1.3.37 に `IOwnedRecordsFieldDesign` (`GetOwnedRecords()` → `OwnedRecordsDesign{Name, Condition}`) と `ModuleDataIO.GetWithOwnedRecordsAsync(module, id)` (宣言を辿って子孫を再帰・親の実値で束縛・ページングなし・ユーザー権限)。`ListFieldDesignBase` は「親と一緒に消える一覧 (DeleteTogether)」を従属として返す (履歴側は DeleteTogether を見ない)。Extras の Gantt/Calendar/TaskBoard デザインも実装。Recorder の読み直しは本体の部品に置換 (自前 Loader 削除)。差分/復元/復活ボタンは `EditHistoryContracts.OwnedRecords` で宣言を列挙。ランタイム側は Extras の `IOwnedRecordsField.ApplyOwnedRecordsAsync` (Gantt/Calendar/TaskBoard/MarkerList が実装・`OwnedRecordsRestore` が ModuleCollection を行 Id で差し替え。表示範囲しか読んでいないフィールドは突き合わせ前に全件読み直す)。MarkerListFieldDesign も宣言、MarkerListField に AllowLoad 追加 (版表示ダイアログで止める)。Example `Project` は Gantt だけで履歴に載る (非表示一覧は不要になり削除)
- **Example のテスト用モジュール**: `Order` (受注: OrderNo/CustomerName/OrderDate/Status/Amount/Note + 明細 Items→`OrderItem` DeleteTogether + LogicalDelete + History) と `OrderItem` (Order Link/ProductName/Qty/UnitPrice + LogicalDelete)。テーブル orders/order_items はローカル SQLite (Design には DDL 無し)。メニュー「受注 (履歴テスト)」→ /Main/Order。履歴モジュール `EditHistory` の詳細に RestoreButton。承認系 (ApprovalRoute 等) は他の確認にも使うので履歴の検証には使わない (ユーザー指示 2026-09-26)
- **復活 (2026-09-26)**: `EditHistoryRestoreButtonField` (履歴モジュール側)。論理削除 (`EditHistoryContracts.IsLogicalDeleteModule`) なら削除の版のスナップショットから親子孫の Id を集め `EditHistoryUndeleteData` (JsonAbstract) を ModuleSubmitData.ExtendedData に載せて送る→Recorder が base の前に本体 `ModuleDataIO.UndeleteAsync` (core 1.3.37・CanDelete+UserWrite で認可) を呼ぶ→ChangeType=Restore。物理削除なら ModuleCreationService で新規モジュールを作り Restorer で反映して SubmitAsync (Add)。明細行の復活は「この版に戻す」の中で: 論理削除の明細は `InsertRowsAsync` に Id 付きで渡すと既存行扱い (IsNewData=false) になるのでそれで行を作り、EditHistoryField._pendingUndeletes に登録→GetSubmitData の ExtendedData で同梱。履歴と退避 (DeleteArchiveFieldDesign) の併用はデザインチェック `EditHistoryFieldDesign:3`
- **未対応 (v1 割り切り)**: 添付ファイルの実体復元、履歴モジュールの雛形生成 (関係モジュール生成は仕様書のみ)
