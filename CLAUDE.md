...
- **形**: 書き込み専用列 2 本 (`DbColumnText` 文章 / `DbColumnVector` = `[0.1,-0.2,…]` JSON 配列テキスト。旧 base64 も Decode は受ける) を同じテーブルに。UI なし (PasswordHashField と同型。`FieldDesignBase` 直下)。本体 (core) は触らない
- **DB 側ベクトル検索 (2026-09-13)**: デザイン `DbColumnVectorSearch` = 距離計算に使うベクトル型列 (bool フラグでなく列名。PG は `search_vector::vector` の生成列、SQL Server 2025 は VECTOR 型列にテキストから暗黙変換で書けるので同名)。`SemanticSearchIndexReader.UsesDbSearch` = 列あり かつ `SupportsDbSearch(type)` (PostgreSQL / SQLServer) のときだけ `SearchAsync` (`BuildSearchSql`: PG `<=>`+limit / SQL Server `VECTOR_DISTANCE('cosine')`+top、`semantic_score` = 1-距離、論理削除は `cast(col as integer)=0`)。失敗は警告ログ→メモリ比較にフォールバック。SQLite で同じデザインは自動でメモリ側 = 可搬。`VectorLiteral` = PG `'[..]'::vector` / SQL Server `CAST('[..]' AS VECTOR(n))`
- **execute_sql の `{embed:…}`**: `RawDataAccessToolSet.SqlPreprocessor` (Func<ds, sql, ct, Task<sql>>) を RawDataAccessAgent が `SemanticSearchToolSet.ExpandEmbeddingsAsync` に結線。SELECT 判定より前に置換 (数値リテラルだけになる)。非対応 DS は例外→AI にエラー (search_records を案内)。ログは展開前の SQL。AI への説明は SemanticSearchToolSet.GetInstructions が DB 検索可能モジュールの表・列・方言ヒント (`DialectHint`) を列挙。テスト = `Test/AI/SemanticSearchDbSearchTest` (文字列検証のみ・実 PG/SQL Server 未検証)
