...
フィールド自体は高さの設定を持たない (RichTextField と同じ)。

- 通常の行: 編集欄は中身に合わせて伸びる (最小 6 行)
- `IsFillAvailable: true` のグリッドの最終行: 残りの高さいっぱいになり、中身は編集欄の中でスクロールする
- 固定したいとき: Grid の行 `Height` を指定する。枠内で内部スクロールになる
