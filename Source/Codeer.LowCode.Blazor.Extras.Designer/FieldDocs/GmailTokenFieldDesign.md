...
他のインフラ (独自に IMailSender を実装したもの) は上位のレイヤで解決するので、このフィールドは不要です。

## セットアップ

**メールのセットアップ** (デザイナ Tools > メールのセットアップ / CLI `mail-setup --gmail-token`) の
「Gmail ユーザートークン欄を追加」でユーザーモジュールにフィールド・レイアウト・列追加 DDL が揃う (任意。Gmail で本人名義に送る場合だけ)。
