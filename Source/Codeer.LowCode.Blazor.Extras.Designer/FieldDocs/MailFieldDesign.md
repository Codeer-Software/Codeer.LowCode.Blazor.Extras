...
| プロパティ | 説明 |
|---|---|
| 自分を差出人にする | ON = 操作ユーザー本人のアドレスが差出人 (サーバーが解決)。OFF = 送信インフラ設定の差出人。**差出人のアドレス指定はできない** (なりすましの構造的排除)。要: デザインの CurrentUser モジュール設定と `Mail.UserEmailFieldName` (既定 "Email")。スクリプトから設定可 |
