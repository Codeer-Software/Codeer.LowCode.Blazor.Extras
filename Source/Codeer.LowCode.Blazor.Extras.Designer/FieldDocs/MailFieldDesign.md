...
| メールインフラ名 | appsettings の Mail.Infras の設定名 (どの送信インフラ・既定差出人を使うか)。**省略可**で、省略 (空) なら既定のインフラを使う (Mail.DefaultInfraName → 先頭)。使うインフラが1つ、あるいは appsettings で既定を決めているなら書かないのが基本形。**デザイン固定** (スクリプトからは変更不可) |
