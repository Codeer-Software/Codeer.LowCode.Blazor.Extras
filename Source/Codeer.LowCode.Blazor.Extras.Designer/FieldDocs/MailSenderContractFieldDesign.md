...
手で置かなくても**メールのセットアップ** (デザイナ Tools > メールのセットアップ / CLI `mail-setup --sender-contract`) の
「差出人契約を追加」でユーザーモジュールに追加できる (メールアドレス・表示名のフィールドを選ぶ。既にあれば触らない)。
システムのアドレスから送るだけなら不要 (セットアップの既定は追加しない)。
