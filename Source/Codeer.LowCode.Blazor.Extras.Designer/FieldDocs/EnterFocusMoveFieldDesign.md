...
このフィールドは外部ライブラリ `Codeer.LowCode.Blazor.Extras` で定義されているため、`Codeer.LowCode.Blazor` リポジトリ内に C# 定義は存在しない。`FieldDesignBase` を直接継承する。共通プロパティは Name のみ（IgnoreModification / OnValidateInput はデザイナ非表示）。
