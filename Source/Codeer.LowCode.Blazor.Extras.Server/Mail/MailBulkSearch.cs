...
namespace Codeer.LowCode.Blazor.Extras.Server.Mail
{
    ...
    public class MailBulkSearch
    {
        ...
        /// <summary>宛先解決の結果 1 行分 (送信対象・除外どちらも)。プレビューが「誰が・なぜ外れたか」を見せるのに使う。</summary>
        ...
        internal class RecipientSet
        {
            ...
            public List<MailBulkRecipient> Recipients => Entries.Where(e => e.Recipient != null).Select(e => e.Recipient!).ToList();
        }

        public async Task<MailSendResult> SendAsync(MailBulkSearchRequest request)
        {
            var set = await ResolveRecipientsAsync(request);
            var recipients = set.Recipients;
        ...
        internal async Task<RecipientSet> ResolveRecipientsAsync(MailBulkSearchRequest request)
        {
            ...
            var paths = names
                ...
                .Concat(new[]
                {
                    ...
                    MailVariableResolver.ParseToken(contract.DisplayName).FieldPath,
                })
            ...
            {
                var (recipient, exclusion, variables) = MailRecipientBuilder.Build(design, row, contract.Email, contract.OptOut, names,
