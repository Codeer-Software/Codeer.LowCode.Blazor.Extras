...
namespace Codeer.LowCode.Blazor.Extras.Designs
{
    ...
    public abstract class ContractFieldDesignBase : FieldDesignBase
    {
        ...
        private const int CodeContractFieldDuplicated = 1;
        private const int CodeRoleRequired = 2;
        ...
        /// <summary>役割プロパティ (プロパティ名 = 役割名) の一覧。チェックとリネーム追従で使う。DB 列の宣言 (DbColumn 属性付き、または候補が DbColumn) は役割ではない。</summary>
        internal IEnumerable<System.Reflection.PropertyInfo> GetRoleProperties()
            => GetType().GetProperties()
                .Where(e => e.DeclaringType != typeof(FieldDesignBase) && e.PropertyType == typeof(string) &&
                            e.GetCustomAttributes(typeof(DesignerAttribute), true).OfType<DesignerAttribute>().Any(a => a.CandidateType != CandidateType.DbColumn) &&
        ...
        public override List<DesignCheckInfo> CheckDesign(DesignCheckContext context)
        {
            ...
            if (ownModule != null && ownModule.Fields.Count(e => e.GetType() == GetType()) > 1)
            {
                ...
                {
                    Code = DesignCheckCode.Create(typeof(ContractFieldDesignBase), CodeContractFieldDuplicated),
            ...
            foreach (var role in GetRoleProperties())
            {
                ...
                if (string.IsNullOrEmpty(fieldName))
                {
                    ...
                    result.Add(new FieldDesignCheckInfo
                    {
                        Code = DesignCheckCode.Create(typeof(ContractFieldDesignBase), CodeRoleRequired),
                        Location = new FieldDesignDataLocation
